...
import {
  ...
  captureHarborPane,
  executeInHarborPane,
  getLiveHarborSession,
  harborSessionExists,
  isInsideHarborSession,
  killHarborSession,
  listHarborWindows,
  sendToHarborPane,
  type SessionInfo,
} from './harbor-session.js';
import { type Config, type DevService, type Script, validateConfig } from './harbor-config.js';
...
async function checkAccess(target: string): Promise<{ allowed: boolean; error?: string }> {
...
function parseWholeNumberOption(
  value: string,
  optionName: string,
  minimum: number
): number {
  ...
  try {
    ...
    for (const folder of folders) {
      if (folder.isDirectory()) {
        ...
        const command = Object.entries(projectCommands).find(([file]) => (
...
export function resolvePackageRoot(entryDir: string): string {
  return fs.existsSync(path.join(entryDir, 'package.json'))
    ? entryDir
    : path.join(entryDir, '..');
}

function _formatSessionContext(session: SessionInfo, currentService?: string): string {
  const currentServiceInfo = currentService ? session.services[currentService] : null;
  const canAccessList = currentServiceInfo?.canAccess || [];
